...
def self_chat(opt):
    ...
    partner1 = opt['partner_model_file_01']
    partner2 = opt['partner_model_file_02']
    partner1_opt_file = opt.get('partner_model_file_01')
    partner2_opt_file = opt.get('partner_opt_file_02')
    ...
    if partner1 is None or partner2 is None:
        ...
        if partner_opt_file_01:
            print(f"WARNING: Loading override opts from: {partner_opt_file_01}")
            with PathManager.open(partner_opt_file_01) as f:
                partner_opt_01 = json.load(f)
        else:
            partner_opt_01 = {}
        partner_opt_01['interactive_mode'] = opt.get('interactive_mode', True)
        print(
            f"WARNING: Setting partner interactive mode to: {partner_opt_01['interactive_mode']}"
        )
        agent1 = create_agent_from_model_file(partner1, partner_opt_01)
        agent1.opt.log("Agent 1 Opt")
        # Self chat with different models
        if partner_opt_file_02:
            print(f"WARNING: Loading override opts from: {partner_opt_file_02}")
            with PathManager.open(partner_opt_file_02) as f:
                partner_opt_02 = json.load(f)
        else:
            partner_opt_02 = {}
        partner_opt_02['interactive_mode'] = opt.get('interactive_mode', True)
        print(
            f"WARNING: Setting partner interactive mode to: {partner_opt_02['interactive_mode']}"
        )
        agent2 = create_agent_from_model_file(partner2, partner_opt_02)
        agent2.opt.log("Agent 2 Opt")



    # Set IDs
    agent0.id = agent0.id + "_0"
    agent1.id = agent1.id + "_1"
    agent2.id = agent2.id + "_2"

    model_id = agent0.id + "_" + agent1.id

    world = create_task(opt, user_agents=[agent0, agent1, agent2])
