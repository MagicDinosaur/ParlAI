...
from parlai.core.worlds import create_task, DialogPartnerWorld, validate, MultiAgentDialogWorld
...
class SelfChatWorld(MultiAgentDialogWorld):
    ...
    def parley(self):
        ...
        if self.contexts:
            ...
            agents = self.agents
            acts[0] = agents[0].act()
            agents[2].observe(validate(acts[0]))
            acts[1] = agents[1].act()
            agents[1].observe(validate(acts[1]))
            acts[2] = agents[2].act()
            agents[0].observe(validate(acts[2]))           
